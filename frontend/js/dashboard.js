...
let map;
let markersLayer; // We use a LayerGroup for markers instead of GeoJSON layer

// 1. Initialize Map (Normal/Light Theme)
function initMap() {
    const delhiBounds = [
        [28.4045, 76.8425], 
        [28.8837, 77.3476]
    ...
    // Using Standard OSM Tiles ("Normal Lovable" look)
...
async function fetchDashboardData() {
    ...
    try {
        ...
        // Render everything
        renderMapMarkers(wardsData); // CHANGED: Using Marker Logic
...
// 3. Render Markers (The "Admin Dashboard" Style)
function renderMapMarkers(geoJsonData) {
    ...
    geoJsonData.features.forEach(feature => {
        ...
        const color = Utils.getRiskColor(props.riskLevel); // Use Utils from common.js
        ...
        const marker = L.circleMarker([lat, lng], {
            ...
            opacity: 0.9,
            fillOpacity: 0.7,
            className: `ward-hotspot ward-hotspot-${riskLevel}` // Applies CSS glow
        });
