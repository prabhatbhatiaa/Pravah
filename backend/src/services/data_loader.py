...
def load_initial_data():
    ...
    1. Load Wards (Base Topology)
    2. Merge Rainfall Data
    3. Merge Drainage Data
    4. Load Historical Complaints
    """
...
def load_complaints_csv(path):
    ...
    try:
        with open(path, mode='r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                # Basic validation to skip corrupted lines
...
def save_complaint_to_csv(complaint: ComplaintCreate):
    ...
    Checks if a newline is needed before appending.
    """
